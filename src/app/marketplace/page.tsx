...
const EXAMPLE_BIDS = [
  {
    id: "B-101",
    title: "50 litres of A2 milk daily, six-month contract",
    postedBy: "Example buyer",
    location: "Gwalior",
  ...
  {
    id: "B-103",
    title: "Organic jaggery supplier wanted, 100 kg a week",
    postedBy: "Example buyer",
    location: "Dabra",
...
export default function MarketplacePage() {
  ...
  const showError = (field: keyof RequirementDraft) => (attempted ? errors[field] : undefined);
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 md:p-8 pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2 max-w-xl">
            Write down exactly what you are buying or selling, then send it to a trader,
            cooperative or FPO on WhatsApp.
          </p>
        </div>
        <Button
          size="lg"
        ...
        >
          {open ? (
            <>
              <X className="w-4 h-4 mr-2" aria-hidden="true" /> Close
            </>
          ) : (
            <>
              <Plus className="w-4 h-4 mr-2" aria-hidden="true" /> Write a requirement
            </>
          )}
        </Button>
      </div>
        ...
        <Card className="mb-8 border-primary/30 border-2">
          ...
          <CardContent className="space-y-5">
            <div
              ...
              role="group"
              aria-label="Are you selling or buying?"
            >
              {(
                [
                  { id: "selling", label: "I am selling" },
                  { id: "buying", label: "I am buying" },
            ...
            <Field label="Goods" htmlFor="product" error={showError("product")}>
              <Input
                id="product"
                value={draft.product}
                onChange={(e) => set("product", e.target.value)}
                placeholder="A2 cow milk, hand-woven baskets, organic jaggery…"
                aria-invalid={Boolean(showError("product"))}
              />
            </Field>

            <div className="grid grid-cols-[1fr_auto] gap-3 items-start">
              <Field label="Quantity" htmlFor="quantity" error={showError("quantity")}>
                <Input
                  id="quantity"
                  type="number"
                  inputMode="numeric"
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              <Field label="Price from (₹)" htmlFor="budgetMin" error={showError("budgetMin")}>
                <Input
                  id="budgetMin"
                  type="number"
                  inputMode="numeric"
              ...
              <Field label="Price to (₹)" htmlFor="budgetMax" error={showError("budgetMax")}>
                <Input
                  id="budgetMax"
                  type="number"
                  inputMode="numeric"
              ...
              <Field
                label={draft.side === "selling" ? "Available until" : "Needed by"}
                htmlFor="needBy"
            ...
            <Field label="Notes" htmlFor="notes" error={showError("notes")}>
              <Textarea
                id="notes"
                value={draft.notes}
                maxLength={MAX_NOTES}
                onChange={(e) => set("notes", e.target.value)}
                placeholder="Grade, packaging, delivery, payment terms — anything a buyer would ask."
              />
            </Field>

            <p className="text-[11px] text-muted-foreground">
              Leaving both price fields blank is fine — the requirement will read “Negotiable”.
              {location ? ` Location will read ${[location.block, location.district].filter(Boolean).join(", ")}.` : ""}
            </p>
          </CardContent>
                ...
                <Card key={r.id} className="flex flex-col border-primary/30">
                  ...
                  <CardContent className="flex-1 space-y-2 text-sm text-muted-foreground">
                    ...
                    {by && (
                      <div className="flex items-center gap-2">
                        ...
                        {r.side === "selling" ? "Available until" : "Needed by"} {by}
                      </div>
                    )}
                    {(r.block || r.district) && (
                      <div className="flex items-center gap-2">
                  ...
                  <CardFooter className="border-t pt-4 gap-2">
                    ...
                    >
                      {copiedId === r.id ? (
                        <>
                          <Check className="w-4 h-4 mr-2" aria-hidden="true" /> Copied
                        </>
                      ) : (
                        <>
      ...
      <div className="flex items-baseline justify-between gap-3 mb-4">
        <h2 className="text-lg font-bold font-heading">What a live board would look like</h2>
      </div>
      <p className="text-sm text-muted-foreground mb-5 max-w-2xl">
        These three are examples, not real listings — there is no counterparty behind them, so
        offering is disabled. They show the shape of the board once a deployment has buyers on it.
      </p>
          ...
          <Card key={bid.id} className="flex flex-col shadow-sm bg-muted/20">
            <CardHeader className="pb-3">
              <div className="flex justify-between items-start mb-2 gap-2">
                ...
                <div className="flex items-center text-xs font-medium text-muted-foreground bg-muted px-2 py-1 rounded-md">
                  <Clock className="w-3 h-3 mr-1" aria-hidden="true" /> {bid.deadline}
                </div>
              </div>
            ...
            <CardContent className="flex-1 space-y-4">
              <div className="space-y-2 text-sm text-muted-foreground">
                ...
                <div className="flex items-center gap-2">
                  <Tag className="w-4 h-4 text-primary/70" aria-hidden="true" /> Budget:{" "}
                  <span className="font-medium text-foreground">{bid.budget}</span>
                </div>
              </div>
            ...
            <CardFooter className="border-t pt-4">
              <Button variant="outline" className="w-full justify-between" disabled>
                Offering opens with a deployment
